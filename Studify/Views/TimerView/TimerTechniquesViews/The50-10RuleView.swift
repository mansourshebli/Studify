...
struct FiftyTenRuleMethodView: View {
    @State private var timer: Timer?
    @State private var timerDuration = 3000.0 // 50 minutes for work
    ...
    func endTimer() {
        timer?.invalidate()
        timerDuration = 3000.0 // Reset to 50 minutes for work
        timerProgress = 1.0 // Reset progress
        isTimerRunning = false
    }

    func startBreak() {
        timerDuration = 600.0 // 10 minutes for break
        showAlert = false
        isTimerRunning = false
        startTimer()
    }
...
// Preview
struct FiftyTenRuleMethodView_Previews: PreviewProvider {
    static var previews: some View {
        FiftyTenRuleMethodView()
    }
}
