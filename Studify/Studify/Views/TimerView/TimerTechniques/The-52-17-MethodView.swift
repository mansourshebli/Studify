...
struct Method5217TimerView: View {
    ...
    @State private var timerDuration = 3120.0 // 52 minutes
    ...
    func endTimer() {
        timer?.invalidate()
        timerDuration = 3120.0 // Reset to 52 minutes
        timerProgress = 1.0 // Reset progress
        isTimerRunning = false
    }

    func startBreak() {
        timerDuration = 1020.0 // 17 minutes break
        showAlert = false
        isBreak = true
        startTimer()
    }
    ...
    func startWork() {
        timerDuration = 3120.0 // 52 minutes work session
        showAlert = false
        isBreak = false
        startTimer()
    }
